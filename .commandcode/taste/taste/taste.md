...
- Prefers using the `/impeccable` skill (and its playbooks) for UI polish and animation work. Confidence: 0.7
- Does not want a `Co-authored-by` trailer (e.g., `Co-authored-by: CommandCodeBot`) in git commit messages; explicitly asked to remove it from a commit. Confidence: 0.9
- Does not want new modules or packages installed just to test/verify; prefers using already-available tools (e.g., curl) instead. Confidence: 0.8
